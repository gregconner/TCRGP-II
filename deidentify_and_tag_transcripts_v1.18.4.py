#!/usr/bin/env python3
"""
De-identify and Tag Transcripts for Research v1.18.4
...
Version: 1.18.4 (GPU support: CUDA + Metal/MPS)
"""
...
class DeIdentifier:
    ...
    def deidentify_text(self, text: str, remove_timestamps: bool = True, format_dialogue: bool = True, 
        ...
        if self.ambiguous_policy == "mark_all":
            try:
                for tok in sorted(self.AMBIGUOUS_NAME_WORDS):
        ...
        # Replace full names — PERFORMANCE FIX v1.18.4
        #
        # Previous versions could "hang" here because they:
        # - built extremely large variant mappings, then
        # - ran multiple full-text regex passes per variant (very slow).
        #
        # New approach:
        # - Only keep variants that actually appear in the current text (substring prefilter)
        # - Build canonical->variants once (avoid nested scans)
        # - Do ONE combined regex pass using alternation (longest-first) + lookup
        #
        # Also: avoid replacing inside bracketed ambiguous tokens like "[Will]" which we intentionally keep.
        canonical_mapping = self.name_detector.get_canonical_mapping()
        deidentified_lower = deidentified.lower()

        canonical_to_variants = {}
        try:
            for var, canon in (canonical_mapping or {}).items():
                canonical_to_variants.setdefault(canon, set()).add(var)
        except Exception:
            canonical_to_variants = {}

        # Map lowercase variant -> code for case-insensitive replacement
        variant_to_code_lc = {}

        def _maybe_add_variant(v: str, code: str) -> None:
            ...
            if len(v) < 3:
                return
            v_lc = v.lower()
            if v_lc not in deidentified_lower:
                return
            variant_to_code_lc[v_lc] = code

        for original, code in person_items:
            _maybe_add_variant(original, code)
            if isinstance(canonical_mapping, dict):
                canon = canonical_mapping.get(original, original)
                for v in canonical_to_variants.get(canon, set()):
                    _maybe_add_variant(v, code)

        # Add misspellings where they actually appear
        try:
            for canon, variants in (COMMON_MISSPELLINGS or {}).items():
                canon_code = None
                for orig, code in person_items:
                    o = orig.lower()
                    if o == canon or canon in o or o in canon:
                        canon_code = code
                        break
                if canon_code:
                    for v in variants:
                        _maybe_add_variant(v, canon_code)
        except Exception:
            pass

        if variant_to_code_lc:
            variants_sorted = sorted(set(variant_to_code_lc.keys()), key=len, reverse=True)
            if variants_sorted:
                name_pat = re.compile(
                    r"(?<!\[)(?<![A-Za-z])(" + "|".join(re.escape(v) for v in variants_sorted) + r")(?![A-Za-z])(?!\])",
                    flags=re.IGNORECASE,
                )

                def _repl(m):
                    return variant_to_code_lc.get(m.group(1).lower(), m.group(0))

                deidentified = name_pat.sub(_repl, deidentified)
        
        # v1.18.4: The combined replacement above already covers canonical + misspelling variants
        # that are present in the text; skip the extra per-variant pass for speed.
...
Examples:
  ...
  python deidentify_and_tag_transcripts_v1.7.0.py --lines-per-page 40
    ...
    print("=" * 80)
    print("DE-IDENTIFY AND TAG TRANSCRIPTS v1.18.4")
    print("=" * 80)
