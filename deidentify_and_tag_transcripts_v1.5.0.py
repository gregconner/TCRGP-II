#!/usr/bin/env python3
"""
De-identify and Tag Transcripts for Research v1.5.0
...
class DeIdentifier:
    ...
    def extract_entities(self, text: str) -> Dict[str, List[str]]:
        ...
        # METHOD 1: Regex patterns for speaker labels (high precision) - IMPROVED v1.5.0
        person_patterns = [
            # Pattern: "FirstName LastName:" at start of line (speaker labels only)
            r'^([A-Z][a-z]+\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)\s*:',
            # Pattern: Single capitalized name at start of line (common first names)
            r'^(Pamela|Amy|Perry|Joy|Gabriel|Miranda|Danae|Vicki|Ricardo|Duran|Lea|Michelena|Anna|Jodi|Pam|Webster|Ariza|Sekiros)\s*:',
        ]
        ...
        # METHOD 1.5: Extract last names mentioned alone (e.g., "Andrews said") - IMPROVED v1.5.0
        last_name_patterns = [
            r'\b(Andrews|Sattler|Burshia|Standing|Zeise|Ariza|Sekiros|Webster|Ho-Chunk|Lesson|Umaha)\s+(?:said|asked|told|mentioned|explained|stated|has|had|was|is|will|would)',
        ]
        for pattern in last_name_patterns:
            ...
            for match in matches:
                ...
                if name not in extracted_names:
                    entities["persons"].append(name)
                    extracted_names.add(name)
                    self.name_detector.add_name(name, "last_name_in_dialogue")
