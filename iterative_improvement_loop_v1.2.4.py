#!/usr/bin/env python3
"""
Iterative Improvement Loop v1.2.4
...
- Auto-detects latest de-identification script version from VERSION file (with override)
- Optional per-second progress ticker for long-running grading (no sleeps)
- Optimized for speed
...
def run_cleaner(transcript_files: List[Path], output_dir: Path, version: str) -> bool:
    ...
    cmd = [
        ...
        "--files", *[str(p) for p in transcript_files],
        "-o", str(output_dir)
    ]
...
def run_grader(raw_file: Path, cleaned_file: Path, mapping_file: Path, tags_file: Path, output_file: Path) -> Dict:
    ...
    cmd = [
        sys.executable,
        "grade_transcript_cleaning_v1.1.2.py",
        str(raw_file),
        str(cleaned_file),
        "-m", str(mapping_file),
        "-t", str(tags_file),
...
def main():
    """Main iterative improvement loop - OPTIMIZED v1.2.0."""
    parser = argparse.ArgumentParser()
    parser.add_argument("--cleaner-version", default=None, help="Override deidentify_and_tag_transcripts version (e.g., 1.17.0). Default: read from VERSION.")
    ...
    print("=" * 80)
    print("ITERATIVE IMPROVEMENT LOOP v1.2.0 (OPTIMIZED)")
    print(f"Using {max(1, args.num_cpus)} CPU cores")
    print("=" * 80)
    ...
    # Initialize - UPDATED v1.2.0 to use latest version automatically
    cleaner_version = (args.cleaner_version or _detect_cleaner_version_from_version_file(script_dir / "VERSION")).strip()
    grader_version = "1.0.0"
    iteration = 0
    previous_scores = []
    no_progress_count = 0
    max_iterations = max(1, args.max_iterations)  # Safety limit
    ...
    while True:
        ...
        # Run grading in parallel using up to N CPUs
        ...
        if num_workers > 1 and len(grading_tasks) > 1:
            ...
            def _tick():
                nonlocal completed, total, started
                elapsed = max(time.time() - started, 1e-6)
                rate = completed / elapsed
                remaining = total - completed
                eta = int(remaining / max(rate, 1e-6))
                sys.stdout.write(f"\r  Grading: {completed}/{total} ({(completed/total*100):5.1f}%) | {rate:5.2f}/s | ETA {eta:4d}s   ")
                sys.stdout.flush()

            prev = _start_per_second_ticker(_tick) if args.progress else None
            try:
                with multiprocessing.Pool(processes=num_workers) as pool:
                    for filename, report in pool.imap_unordered(run_grader_wrapper, grading_tasks):
                        ...
                        # Keep a per-item confirmation too (useful when stdout is captured)
