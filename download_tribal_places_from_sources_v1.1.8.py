#!/usr/bin/env python3
"""
Download Comprehensive Tribal Place Names from Public Sources v1.1.8
...
# Optional sink for non-scrolling 3-line progress display in the parent DB builder.
# Expected: has method update_subtask(done=?, total=?, detail=?)
PROGRESS_SINK = None
...
def _sink_active() -> bool:
    return PROGRESS_SINK is not None and hasattr(PROGRESS_SINK, "update_subtask")

# When the DB builder invokes this downloader, we must NOT call back into the builder
# (that creates a circular dependency and an infinite loop). The builder sets this.
DISABLE_BUILDER_CURATED = os.environ.get("TCRGP_DISABLE_BUILDER_CURATED", "").strip() in ("1", "true", "TRUE", "yes", "YES")
...
def _download_bytes_cached(url: str, label: str, timeout_s: float, attempts: int, refresh: bool = False) -> bytes:
    cache_bin, cache_meta = _cache_paths(url)
    if cache_bin.exists() and not refresh:
        try:
            data = cache_bin.read_bytes()
            if _sink_active():
                PROGRESS_SINK.update_subtask(done=1, total=1, detail=f"{label}: cache hit ({len(data)/1024/1024:.1f}MB)")
            else:
    ...
    def _tick():
        ...
        if total:
            ...
            eta = int((total - downloaded) / max(speed, 1e-6))
            msg = (f"    ⏬ {label}: {pct:5.1f}% | "
                   f"{downloaded/1024/1024:8.1f}MB / {total/1024/1024:8.1f}MB | "
                   f"{speed/1024/1024:5.2f} MB/s | ETA: {eta:4d}s")
        else:
            msg = (f"    ⏬ {label}: {downloaded/1024/1024:8.1f}MB | "
    ...
    # If a parent is managing a 3-line no-scroll UI, don't print and don't run our own ticker.
    prev = None if _sink_active() else _start_per_second_progress_timer(_tick)
    try:
        ...
        with _fast_urlopen(req, timeout_s=timeout_s, attempts=attempts) as resp:
            ...
            while True:
                ...
                if _sink_active():
                    PROGRESS_SINK.update_subtask(done=downloaded, total=total, detail=label)
            data = buf.getvalue()
        try:
...
def _fast_urlopen(req_or_url, timeout_s: float, attempts: int = 2):
    ...
    - No sleeps / no backoff.
    - At most `attempts` tries.
    - Small per-attempt timeout.
    """
    last_exc = None
...
def _parse_dbf_records(dbf_bytes: bytes) -> List[Dict[str, str]]:
    """Minimal DBF (dBase III/IV) parser to extract records without external deps."""
    if len(dbf_bytes) < 32:
        return []

    # Header
    num_records = int.from_bytes(dbf_bytes[4:8], "little", signed=False)
    header_len = int.from_bytes(dbf_bytes[8:10], "little", signed=False)
    record_len = int.from_bytes(dbf_bytes[10:12], "little", signed=False)
    ...
    while offset < header_len:
        if dbf_bytes[offset] == 0x0D:
            break
        field_desc = dbf_bytes[offset:offset+32]
        name = field_desc[0:11].split(b"\x00", 1)[0].decode("ascii", errors="ignore").strip()
    ...
    for i in range(num_records):
        pos = rec_start + i * record_len
        if pos + record_len > len(dbf_bytes):
            break
        ...
        for name, ftype, flen in fields:
            raw = rec[p:p+flen]
            p += flen
            if ftype in ("C", "M"):  # character/memo pointer
...
def download_census_tiger_layer(year: int, layer: str, out_type: str, refresh: bool = False) -> List[Dict]:
    ...
    try:
        data = _download_bytes_cached(url, f"TIGER {layer_upper}", timeout_s=30, attempts=2, refresh=refresh)
    except Exception as e:
        done = True
...
def download_census_tiger_aiannh(year: int = 2023, refresh: bool = False) -> List[Dict]:
    """AIANNH (American Indian/Alaska Native/Native Hawaiian Areas) names from TIGER/Line."""
    return download_census_tiger_layer(year=year, layer="AIANNH", out_type="aiannh", refresh=refresh)

def download_census_tiger_aitsn(year: int = 2023, refresh: bool = False) -> List[Dict]:
    """AITSN (American Indian Tribal Subdivisions) names from TIGER/Line (when available)."""
    return download_census_tiger_layer(year=year, layer="AITSN", out_type="tribal_subdivision", refresh=refresh)

def download_census_tiger_ttract(year: int = 2023, refresh: bool = False) -> List[Dict]:
    """TTRACT (Tribal Census Tracts) names from TIGER/Line (when available)."""
    return download_census_tiger_layer(year=year, layer="TTRACT", out_type="tribal_tract", refresh=refresh)

def download_census_tiger_tbg(year: int = 2023, refresh: bool = False) -> List[Dict]:
    """TBG (Tribal Block Groups) names from TIGER/Line (when available)."""
    return download_census_tiger_layer(year=year, layer="TBG", out_type="tribal_block_group", refresh=refresh)
...
def download_gnis_ftp() -> List[Dict]:
    ...
    try:
        ...
        for url in gnis_urls:
            try:
                print(f"    Trying: {url}")
                # Fast-fail: 2 attempts, short timeout
                with _fast_urlopen(url, timeout_s=5, attempts=2) as response:
                    data = response.read()
...
def download_gnis_ld_sparql() -> List[Dict]:
    ...
    try:
        ...
        # Fast-fail: 2 attempts, short timeout
        with _fast_urlopen(url, timeout_s=8, attempts=2) as response:
...
def download_datagov_gnis() -> List[Dict]:
    ...
    try:
        ...
        with _fast_urlopen(show_url, timeout_s=10, attempts=2) as response:
...
def download_gnis_national_file() -> List[Dict]:
    ...
    try:
        ...
        # Download the file
        try:
            # Fast-fail: 2 attempts, short timeout (server has been flaky)
            with _fast_urlopen(url, timeout_s=15, attempts=2) as response:
                data = response.read()
                print(f"  ✓ Downloaded {len(data) / 1024 / 1024:.1f} MB")
...
def download_gnis_state_files() -> List[Dict]:
    ...
    # Fast mode: do NOT spam retries/long loops; try only a couple states quickly.
    max_states = 2
    print(f"  Attempting to download state files (fast mode): {max_states} states...")
    
    for idx, (state_code, state_name) in enumerate(priority_states.items()):
        if idx >= max_states:
            break
        try:
            # Single URL pattern (fast mode)
            urls = [f"https://geonames.usgs.gov/docs/stategaz/{state_code}_Features.zip"]
            ...
            if isinstance(data, list):
                ...
                def update_progress_display():
                    ...
                    if current_elapsed > 0:
                        ...
                        if _sink_active():
                            # Keep detail stable for good ETA; progress numbers come from done/total.
                            PROGRESS_SINK.update_subtask(done=current_completed, total=total_tribes, detail="EPA details")
                        else:
                            if sys.stdout.isatty():
                        ...
                        if _sink_active():
                            PROGRESS_SINK.update_subtask(done=0, total=total_tribes, detail="EPA details")
                        else:
                            if sys.stdout.isatty():
                ...
                # If parent has a 3-line UI, don't steal SIGALRM; just update on completions.
                ...
                try:
                    ...
                    if _sink_active():
                        PROGRESS_SINK.update_subtask(done=total_tribes, total=total_tribes, detail=f"EPA details complete | places {len(places)}")
                    else:
...
def download_comprehensive_tribal_place_list(mode: str = "fast", year: int = 2023) -> List[Dict]:
    ...
    # IMPORTANT: when called from the DB builder, do NOT call back into the builder (circular dependency).
    if not DISABLE_BUILDER_CURATED:
        ...
        print("\n4. Skipping builder-curated list (disabled to avoid circular dependency)")

    if mode == "full":
        print("\n5. Downloading Census TIGER tribal geography layers (tracts/block groups, if available)...")
        # These layers are not always present for every year; we try and move on quickly if missing.
        for fn, label in [
            (download_census_tiger_ttract, "TTRACT"),
            (download_census_tiger_tbg, "TBG"),
        ]:
            try:
                layer_places = fn(year)
                if layer_places:
                    print(f"  ✓ Got {len(layer_places)} places from Census {label}")
                    all_places.extend(layer_places)
            except Exception:
                pass

        # Then try GNIS-LD / National Map / GNIS direct (often down) and/or huge downloads
        print("\n6. Downloading GNIS from Data.gov (CKAN resources; can be large)...")
        datagov_places = download_datagov_gnis()
        if datagov_places:
            print(f"  ✓ Got {len(datagov_places)} places from Data.gov")
            all_places.extend(datagov_places)

        print("\n7. Attempting to download from GNIS-LD (Linked Data) service...")
        gnis_ld_places = download_gnis_ld_sparql()
        if gnis_ld_places:
            print(f"  ✓ Got {len(gnis_ld_places)} places from GNIS-LD")
            all_places.extend(gnis_ld_places)

        print("\n8. Attempting to download from The National Map Services...")
        ...
        print("\n9. Attempting to download from USGS GNIS National File...")
        ...
        print("\n10. Trying other alternative sources...")
...
def get_comprehensive_curated_tribal_places() -> List[Dict]:
    ...
    try:
        ...
        db_builder_path = Path(__file__).parent / "build_name_location_database_v1.2.1.py"
