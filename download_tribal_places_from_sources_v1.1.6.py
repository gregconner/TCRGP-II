#!/usr/bin/env python3
"""
Download Comprehensive Tribal Place Names from Public Sources v1.1.6
            ...
            if isinstance(data, list):
                ...
                def update_progress_display():
                    ...
                    if current_elapsed > 0:
                        ...
                        if _sink_active():
                            PROGRESS_SINK.update_subtask(
                                done=current_completed,
                                total=total_tribes,
                                detail=f"EPA details ({progress_pct:5.1f}%): {current_completed}/{total_tribes} | places {current_places}"
                            )
                        else:
                            if sys.stdout.isatty():
                        ...
                        if _sink_active():
                            PROGRESS_SINK.update_subtask(done=0, total=total_tribes, detail="EPA details: starting")
                        else:
                            if sys.stdout.isatty():
