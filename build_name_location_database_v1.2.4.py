#!/usr/bin/env python3
"""
Build Name and Location Database v1.2.4

ENHANCED v1.2.4 - Most Robust System:
...
class OverallProgress:
    ...
    def print_tick(self):
        ...
        if overall > 0.005:
            ...
            eta_s = max(int(total_est - elapsed), 0)
        else:
            eta_s = None

        if self.stage_total and self.stage_total > 0:
            stage_pct = (self.stage_done / self.stage_total) * 100
            stage_part = f"{self.stage_done:,}/{self.stage_total:,} ({stage_pct:5.1f}%)"
        else:
            stage_part = f"{self.stage_done:,}/?"

        overall_pct = overall * 100
        eta_part = f"ETA {eta_s//60:02d}:{eta_s%60:02d}" if eta_s is not None else "ETA ??"
        rate = (self.stage_done / stage_elapsed) if stage_elapsed > 0 else 0.0

        msg = (
            f"[DB BUILD] Stage {self.stage_idx+1}/{len(self.stages)}: {self.stage_name}"
            f"{(' | ' + self.stage_detail) if self.stage_detail else ''}"
            f" | stage {stage_part} | overall {overall_pct:5.1f}% | {eta_part}"
            f" | stage_rate {rate:6.2f}/s | elapsed {int(elapsed//60):02d}:{int(elapsed%60):02d}"
        )

        # Always visible: newline for non-TTY (logs), carriage update for TTY
...
def populate_database(refresh: bool = False):
    ...
    try:
        ...
        tribal_names, member_names, _tribal_places_seed = download_comprehensive_tribal_data()
        ...
        tribal_places = download_tribal_reservations_comprehensive()
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("Building Name and Location Database v1.2.4")
