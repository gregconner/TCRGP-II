#!/usr/bin/env python3
"""
Build Name and Location Database v1.3.5

ENHANCED v1.3.5 - Most Robust System:
...
- Progress monitor is now a separate process (`progress_monitor_v1.0.0.py`) driven by a JSON state file.
  This guarantees per-second updates even if other code touches SIGALRM in the main process.
...
PROGRESS_STATE_PATH = Path(__file__).parent / "build_db_progress.json"
...
class OverallProgress:
    ...
    def __init__(self, stages: List[Tuple[str, float]]):
        ...
        self._started_at = self.start_t

    def write_state(self, done: bool = False):
        """Write a compact JSON state snapshot for the external monitor."""
        try:
            now = time.time()
            overall = self._overall_fraction() * 100.0

            # derive ETAs similarly to display
            elapsed = max(now - self.start_t, 1e-6)
            if overall > 0.002:
                total_est = elapsed / (overall / 100.0)
                overall_eta = max(total_est - elapsed, 0.0)
            else:
                overall_eta = 60.0
            ...
            if self.task_total and self.task_total > 0 and self.task_done > 0:
                ...
                if rate > 0:
                    task_eta = max((self.task_total - self.task_done) / rate, 0.0)
            sub_eta = None
            if self.sub_total and self.sub_total > 0 and self.sub_done > 0:
                ...
                if rate > 0:
                    sub_eta = max((self.sub_total - self.sub_done) / rate, 0.0)

            task_pct = 0.0
            if self.task_total and self.task_total > 0:
                task_pct = min(max(self.task_done / self.task_total * 100.0, 0.0), 100.0)
            sub_pct = 0.0
            if self.sub_total and self.sub_total > 0:
                sub_pct = min(max(self.sub_done / self.sub_total * 100.0, 0.0), 100.0)

            PROGRESS_STATE_PATH.write_text(json.dumps({
                "pid": os.getpid(),
                "started_at": self._started_at,
                "updated_at": now,
                "overall_pct": float(overall),
                "overall_eta_s": float(overall_eta) if overall_eta is not None else None,
                "task_pct": float(task_pct),
                "task_eta_s": float(task_eta) if task_eta is not None else None,
                "sub_pct": float(sub_pct),
                "sub_eta_s": float(sub_eta) if sub_eta is not None else None,
                "task_name": (self.stage_name or "").strip(),
                "sub_name": (self.sub_detail or "").strip(),
                "done": bool(done),
    ...
    def begin(self):
        # External monitor drives display; we just publish state.
        self.write_state(done=False)

    def end(self):
        self.write_state(done=True)
        ...
        try:
            if self.sub_total and self.sub_total > 0 and self.sub_done > 0:
                if (self.task_total is None) or (self.task_total <= 1):
                    self.task_total = int(self.sub_total)
                    self.task_done = int(self.sub_done)
                    self._task_mirrors_subtask = True
                if self._task_mirrors_subtask:
                    self.task_total = int(self.sub_total)
                    self.task_done = int(self.sub_done)
                if (self.stage_total is None) or (self.stage_total <= 1):
                    self.stage_total = int(self.sub_total)
                    self.stage_done = int(self.sub_done)
                if self._task_mirrors_subtask:
                    self.stage_total = int(self.sub_total)
                    self.stage_done = int(self.sub_done)
        except Exception:
...
def populate_database(refresh: bool = False):
    ...
    # Launch external, non-scrolling 3-line progress monitor (best-effort).
    try:
        monitor = Path(__file__).parent / "progress_monitor_v1.0.0.py"
        if monitor.exists():
            subprocess.Popen([sys.executable, str(monitor), "--state", str(PROGRESS_STATE_PATH)])
    except Exception:
        pass
