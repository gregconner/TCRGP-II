#!/usr/bin/env python3
"""
Build Name and Location Database v1.3.13

ENHANCED v1.3.13 - Most Robust System:
...
- Fix: suppress ALL subtask printing when progress sink is active (3-line UI only).
- Improve ETA accuracy: task totals are set correctly and subtask timers don’t reset every update.
- Fix logic: if SUBTASK > 0%, TASK and OVERALL are forced > 0% (no “0% with progress” contradictions).
- Never show “ETA ??”: always give a best-effort ETA (measured rate when possible, otherwise stage defaults).
- Fix: load tribal downloader module with a unique module name (prevents Python import caching from loading an older variant that prints scrolling progress).
- Guarantee: stdout is filtered so ONLY the 3 progress lines are ever emitted (no stray subtask/progress prints).
- Fix: TASK/OVERALL % continuously track SUBTASK % during download-style subtasks (no “stuck at 0.17%”).
- Fix: keep OVERALL/TASK monotonic by treating 1/1 cache-hit subtasks as ~1% progress, not 100%.
- Fix: progress ticker cannot be overridden—SIGALRM handlers are chained safely (no recursion) and ITIMER_REAL is kept active.
- Fix: nested downloader progress (“⏳ PROGRESS: [x/y] …”) is parsed from stdout and fed into SUBTASK so the 3-line UI always reflects what’s actually running.
- Fix: nested PROGRESS regex corrected (previously over-escaped, so it didn’t match).
- Fix: OVERALL/TASK percentages are monotonic (never decrease) even when subtasks switch/reset totals.
- Fix: break circular dependency between DB builder and tribal downloader (downloader no longer calls back into builder when invoked by builder).
- Fix: ensure disable flag is set BEFORE importing the downloader (and enforced via module global), so it can’t import the builder for curated lists.
...
class _FilteredStdout:
    """Allow only the 3 progress lines (and ANSI cursor controls) to reach stdout.

    Everything else is redirected to a detail log to prevent terminal scrolling/noise.
    """
    def __init__(self, underlying, detail_log, on_progress_line=None):
        self._u = underlying
        self._detail = detail_log
        self._buf = ""
        self._on_progress_line = on_progress_line
    ...
    def write(self, s):
        if not s:
            return 0
        # Allow ANSI cursor controls and our 3 lines only.
        if s.startswith("\x1b") or s.startswith("OVERALL") or s.startswith("TASK") or s.startswith("SUBTASK") or s.startswith("\r\x1b"):
            return self._u.write(s)

        # Capture and parse nested progress lines (e.g., "⏳ PROGRESS: [ 123/576] ...") so our unified
        # 3-line progress UI reflects what is actually running, even in nested libraries/scripts.
        try:
            self._buf += s
            while "\n" in self._buf:
                line, self._buf = self._buf.split("\n", 1)
                if self._on_progress_line:
                    self._on_progress_line(line)
        except Exception:
            pass
        try:
            if self._detail:
                self._detail.write(s)
                self._detail.flush()
        except Exception:
            pass
        return len(s)
...
class OverallProgress:
    ...
    def __init__(self, stages: List[Tuple[str, float]]):
        ...
        self._task_mirrors_subtask = False
        self._max_overall_pct = 0.0
        self._max_task_pct = 0.0

    def begin(self):
        # Make SIGALRM nesting-safe: chain any other SIGALRM handlers and keep ITIMER_REAL active.
        # This prevents submodules from breaking the required per-second progress output.
        self._orig_signal_fn = signal.signal
        self._orig_setitimer_fn = signal.setitimer
        self._orig_sigalrm_handler = signal.getsignal(signal.SIGALRM)
        self._tick_fn = lambda: self.print_tick()

        chained = {"other": self._orig_sigalrm_handler}

        def _our_handler(_sig, _frame):
            try:
                self._tick_fn()
            except Exception:
                pass
            h = chained.get("other")
            try:
                # Avoid self-recursion if someone tries to set SIGALRM to our handler.
                if callable(h) and h is not _our_handler:
                    h(_sig, _frame)
            except Exception:
                pass
        ...
        def _signal_wrapper(sig, handler):
            if sig == signal.SIGALRM:
                # Chain: store "other" handler, keep ours installed.
                if handler is _our_handler:
                    # Don't chain to self.
                    return self._orig_signal_fn(sig, _our_handler)
                chained["other"] = handler
                return self._orig_signal_fn(sig, _our_handler)
            return self._orig_signal_fn(sig, handler)

        def _setitimer_wrapper(which, seconds, interval=0.0):
            # Always ensure our per-second ticker remains active.
            if which == signal.ITIMER_REAL:
                try:
                    prev = self._orig_setitimer_fn(which, seconds, interval)
                except Exception:
                    prev = None
                # If someone tries to disable/shorten the ticker, immediately restore ours.
                try:
                    self._orig_setitimer_fn(signal.ITIMER_REAL, 1.0, 1.0)
                except Exception:
                    pass
                return prev
            return self._orig_setitimer_fn(which, seconds, interval)

        signal.signal = _signal_wrapper  # type: ignore
        signal.setitimer = _setitimer_wrapper  # type: ignore

        # Install ours and start ticker.
        # Install ours and start ticker (use original fns to avoid wrapper self-chaining).
        self._orig_signal_fn(signal.SIGALRM, _our_handler)
        self._orig_setitimer_fn(signal.ITIMER_REAL, 1.0, 1.0)
        self._prev_alarm = self._orig_sigalrm_handler

    def end(self):
        try:
            # restore signal functions
            if hasattr(self, "_orig_signal_fn"):
                signal.signal = self._orig_signal_fn  # type: ignore
            if hasattr(self, "_orig_setitimer_fn"):
                signal.setitimer = self._orig_setitimer_fn  # type: ignore
            # restore SIGALRM handler and disable timer
            if hasattr(self, "_orig_sigalrm_handler"):
                signal.signal(signal.SIGALRM, self._orig_sigalrm_handler)
            try:
    ...
    def next_stage(self, name: str, weight: float, total: Optional[int] = None, detail: str = ""):
        ...
        self._task_mirrors_subtask = False
        # keep monotonic across stages too
        self._max_overall_pct = max(self._max_overall_pct, self._overall_fraction() * 100.0)
        self._max_task_pct = 0.0
    ...
    def update(self, done: Optional[int] = None, total: Optional[int] = None, detail: Optional[str] = None):
        ...
        if detail is not None:
            self.stage_detail = detail
            self.task_detail = detail
        # If the task has been explicitly updated, stop mirroring subtask into task/stage.
    ...
    def update_subtask(self, done: Optional[int] = None, total: Optional[int] = None, detail: Optional[str] = None):
        # Reset subtask timer only when starting a new subtask (done resets) or total changes.
        if total is not None:
            try:
                if self.sub_total != int(total):
                    self._sub_started = time.time()
            except Exception:
                self._sub_started = time.time()
        if done is not None:
            try:
                if int(done) < int(self.sub_done):
                    self._sub_started = time.time()
            except Exception:
                self._sub_started = time.time()
        if done is not None:
        ...
        # Critical logic: if a subtask is progressing, task+stage must also be progressing.
        # We use subtask totals as a proxy for task totals while the task is "download-ish".
        try:
            if self.sub_total and self.sub_total > 0 and self.sub_done > 0:
                # If this subtask is a trivial 1/1 (common for cache hits), do NOT treat it as 100% of the stage.
                mirror_total = int(self.sub_total)
                mirror_done = int(self.sub_done)
                if mirror_total == 1 and mirror_done == 1:
                    mirror_total = 100
                    mirror_done = 1
                if (self.task_total is None) or (self.task_total <= 1):
                    self.task_total = mirror_total
                    self.task_done = mirror_done
                    self._task_mirrors_subtask = True
                if self._task_mirrors_subtask:
                    self.task_total = mirror_total
                    self.task_done = mirror_done
                if (self.stage_total is None) or (self.stage_total <= 1):
                    self.stage_total = mirror_total
                    self.stage_done = mirror_done
                if self._task_mirrors_subtask:
                    self.stage_total = mirror_total
                    self.stage_done = mirror_done
        except Exception:
    ...
    def print_tick(self):
        ...
        # Default per-stage ETA guesses (seconds) used before we have enough signal.
        default_stage_sec = [60, 30, 120, 5, 2, 180, 90, 5]
        # ETA from overall fraction when possible; otherwise fall back to stage defaults.
        if overall > 0.002:
            total_est = elapsed / overall
            eta_s = max(total_est - elapsed, 0.0)
        else:
            # Remaining = current-stage fallback + future-stage fallbacks
            idx = max(self.stage_idx, 0)
            remaining = float(default_stage_sec[min(idx, len(default_stage_sec)-1)])
            for j in range(idx + 1, len(self.stages)):
                remaining += float(default_stage_sec[min(j, len(default_stage_sec)-1)])
            eta_s = remaining

        overall_pct = overall * 100.0
        if overall_pct < self._max_overall_pct:
            overall_pct = self._max_overall_pct
        else:
            self._max_overall_pct = overall_pct

        # Task ETA (prefer measured rate; else use subtask ETA; else stage default)
        task_eta = None
        ...
        if task_eta is None and sub_eta is not None:
            task_eta = sub_eta
        if task_eta is None:
            idx = max(self.stage_idx, 0)
            task_eta = float(default_stage_sec[min(idx, len(default_stage_sec)-1)])
        ...
        if self.task_total and self.task_total > 0:
            ...
            if tpct < self._max_task_pct:
                tpct = self._max_task_pct
            else:
                self._max_task_pct = tpct
        ...
        if self.sub_total and self.sub_total > 0:
            ...
            # If we can't estimate subtask ETA yet, provide a small fallback guess.
            if sub_eta is None:
                sub_eta = 30.0
            sub_line = f"SUBTASK  {spct:6.2f}% | ETA {_fmt_hhmmss(sub_eta)} | {self.sub_detail}".strip()
        else:
            # If no subtask total, still provide a fallback ETA guess.
            if sub_eta is None:
                sub_eta = 30.0
...
def download_tribal_reservations_comprehensive(progress: Optional[OverallProgress] = None) -> List[dict]:
    ...
    try:
        ...
        try:
            ...
            candidate_paths = [
                Path(__file__).parent / "download_tribal_places_from_sources_v1.1.9.py",
                Path(__file__).parent / "download_tribal_places_from_sources_v1.1.8.py",
                Path(__file__).parent / "download_tribal_places_from_sources_v1.1.7.py",
                Path(__file__).parent / "download_tribal_places_from_sources_v1.1.6.py",
            ...
            if download_script and download_script.exists():
                ...
                # Set disable flag BEFORE importing the downloader so it can’t call back into this builder.
                try:
                    os.environ["TCRGP_DISABLE_BUILDER_CURATED"] = "1"
                except Exception:
                    pass
                # Use a unique module name to avoid sys.modules caching an older downloader version.
                mod_name = f"tribal_downloader_{download_script.stem}_{int(download_script.stat().st_mtime)}"
                if mod_name in sys.modules:
                    del sys.modules[mod_name]
                spec = importlib.util.spec_from_file_location(mod_name, download_script)
                ...
                # Also enforce via module global (in case it computed env at import time in older variants).
                try:
                    setattr(tribal_downloader, "DISABLE_BUILDER_CURATED", True)
                except Exception:
                    pass
...
def populate_database(refresh: bool = False):
    ...
    try:
        # Always redirect print() to a detail log so stdout stays strictly 3-line progress only.
    ...
    # Filter sys.stdout too: some subcomponents write directly to sys.stdout (not print()).
    orig_stdout = sys.stdout
    try:
        # Parse nested progress lines into our subtask progress.
        progress_line_re = re.compile(r"\bPROGRESS:\s*\[\s*(\d+)\s*/\s*(\d+)\s*\]")

        def _on_progress_line(line: str):
            m = progress_line_re.search(line or "")
            if not m:
                return
            try:
                done = int(m.group(1))
                total = int(m.group(2))
                if total > 0:
                    # keep the label stable for good ETA; numbers come from done/total
                    prog.update_subtask(done=done, total=total, detail="nested download")
            except Exception:
                return

        sys.stdout = _FilteredStdout(orig_stdout, detail_log, on_progress_line=_on_progress_line)
    except Exception:
        pass
    ...
    try:
        ...
        prog.update_subtask(done=0, total=max(1, total_seed), detail="insert tribal+member names")
        native_count = 0
        member_count = 0
        done_seed = 0
        ...
        for name_data in tribal_names:
            ...
            if done_seed % 200 == 0:
                ...
                prog.update_subtask(done=done_seed)
