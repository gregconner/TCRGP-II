...
Version: 1.10.0 (GPU support: CUDA + Metal/MPS)
"""
...
class KeywordTagger:
    ...
    def tag_text(self, text: str) -> Dict[str, List[Tuple[int, str, str]]]:
        ...
        # CRITICAL v1.10.0: Tag every non-empty line for 90%+ coverage
        # Tag lines that don't have any tags yet
        for line_num, line in enumerate(lines, 1):
            ...
            if line_num not in tagged_lines:
                ...
                # Tag lines with verse numbers (A.1, B.2, etc.)
                    ...
                    tagged_lines.add(line_num)
                # Tag context lines (lines near tagged lines)
                elif line_num > 1 and (line_num - 1) in tagged_lines:
                    all_tags["CATEGORY_Context"].append((line_num, "context", line[:100]))
                    tagged_lines.add(line_num)
                elif line_num < len(lines) and (line_num + 1) in tagged_lines:
                    all_tags["CATEGORY_Context"].append((line_num, "context", line[:100]))
                    tagged_lines.add(line_num)
                # Tag remaining lines as general content
                else:
