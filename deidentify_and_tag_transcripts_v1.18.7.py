#!/usr/bin/env python3
"""
De-identify and Tag Transcripts for Research v1.18.7
...
Version: 1.18.7 (GPU support: CUDA + Metal/MPS)
"""
...
def remove_webvtt_timestamps(text: str) -> str:
    ...
    # v1.17.8: Some DOCX extractions drop the HH:MM:SS and leave only millisecond fragments like:
    #   ".090 --> .280"
    # These are always timestamp artifacts and should be removed.
    text = re.sub(r'^\.\d{3}\s*-->\s*\.\d{3}\s*$', '', text, flags=re.MULTILINE)
...
def parse_non_webvtt_with_timestamps(text: str) -> Tuple[str, List[Dict[str, str]]]:
    ...
    current_dialogue = []
    # v1.18.0: Speaker IDs for timestamp-only transcripts should NOT collide with person codes (#A/#B...).
    # Keep internal speaker markers as Person_1/Person_2; the citation formatter will emit [A.x]/[B.x].
    last_speaker = None
        ...
        if timestamp_match:
            ...
            if current_timestamp and current_dialogue:
                ...
                if dialogue_text:
                    ...
                    # For now, alternate between Person_1 and Person_2 (simple heuristic).
...
def format_with_citation_system(
    ...
    # v1.18.0: Keep internal speaker markers as Person_1/Person_2/... (they never appear in final body).
    # Citation speaker letters (A/B/AA/...) are assigned deterministically by Person_N order.
    speaker_codes = set()
    for raw in lines:
    ...
    for line in lines:
        ...
        # Skip lines that are just "Person_X." (standalone speaker markers without dialogue)
...
class DeIdentifier:
    ...
    def __init__(self, use_spacy: bool = True, use_database: bool = True):
        ...
        # v1.18.1: Words that can be normal English AND human names. If they appear capitalized
        # and get flagged by extraction heuristics, we bracket them like "[Will]" instead of coding.
        self.ambiguous_name_tokens = set()  # stores Title-case tokens to bracket in output
        self.AMBIGUOUS_NAME_WORDS = {
            # Modals / months / common words frequently mis-read as names:
            "Will", "May", "June", "April", "August", "Mark", "Bill", "Hope", "Faith", "Rose", "Summer",
            "Autumn", "Pat", "Guy", "Chase", "Grant", "Justice", "Hunter", "Joy", "Sunny", "Amber",
        }
        # v1.18.2: Controls how we handle ambiguous common-word tokens (Will/May/etc.).
        # - mark_all: bracket ANY capitalized occurrence ("Will is important" -> "[Will] is important")
        # - pos_based: bracket only when spaCy/NER suggests it's name-like (PROPN / PERSON span)
        self.ambiguous_policy = "mark_all"
        
        # NEW v1.16.0: Load name and location database
        self.db_conn = None
        self.use_database = use_database
        self.tribe_terms = set()  # Lowercased tribe/ethnonym strings from DB
        if use_database:
            try:
                import sqlite3
                ...
                if db_path.exists():
                    ...
                    # v1.18.0: Load tribe/ethnonym terms from the DB (no hardcoding).
                    self.tribe_terms = self._load_tribe_terms_from_db()
                else:
    ...
    def _load_tribe_terms_from_db(self) -> set:
        """
        Build a set of tribe / ethnonym terms from the SQLite DB.
        Source columns:
        - tribal_place_names.tribe
        - place_names.tribal_affiliation
        We also split multi-tribe strings like "Assiniboine/Sioux" into tokens.
        """
        terms = set()
        if not (self.use_database and self.db_conn):
            return terms
        try:
            cur = self.db_conn.cursor()
            vals = []
            for (tbl, col) in [("tribal_place_names", "tribe"), ("place_names", "tribal_affiliation")]:
                try:
                    cur.execute(f"SELECT DISTINCT {col} FROM {tbl} WHERE {col} IS NOT NULL AND TRIM({col}) != ''")
                    vals.extend([r[0] for r in cur.fetchall() if r and r[0]])
                except Exception:
                    continue
            ...
            for v in vals:
                ...
                s_low = s.lower()
                terms.add(s_low)
                for tok in _re.split(r"[/,;&]|\\band\\b", s_low):
                    tok = tok.strip()
                    if len(tok) >= 3:
                        terms.add(tok)
        except Exception:
    ...
    def _is_tribe_term(self, s: str) -> bool:
        if not s:
            return False
        s_low = str(s).strip().lower()
        if not s_low:
            return False
        if s_low in self.tribe_terms:
            return True
        for part in re.split(r"[/,;&]|\\band\\b", s_low):
            part = part.strip()
            if part and part in self.tribe_terms:
                return True
        return False

    def _fix_common_tribe_misspellings(self, text: str) -> str:
        """
        v1.18.0: Lightweight cleanup for common tribe self-ID typos seen in transcripts.
        This is intentionally narrow and only fires in obvious tribe-list contexts.
        """
        try:
            ...
            for line in lines:
                s = line
                low = s.lower()
                # If the line is listing tribes (contains known tribe terms), apply a couple fixes.
                if "lakota" in low:
                    # "Sue" is a common transcription error for "Sioux" in tribe lists.
                    if "sioux" in self.tribe_terms:
                        s = re.sub(r"\bSue\b", "Sioux", s)
                    # "Hug Papapa" -> "Hunkpapa" (common variant); only in Lakota context.
                    s = re.sub(r"\bHug\s+Papapa\b", "Hunkpapa", s, flags=re.IGNORECASE)
                out.append(s)
            return "\n".join(out)
        except Exception:
            return text

    def _collect_ambiguous_tokens_pos_based(self, text: str):
        """
        Collect ambiguous tokens for bracketing using spaCy context.
        We mark a token if it is:
        - in AMBIGUOUS_NAME_WORDS
        - Title-cased in the source text
        - and spaCy suggests it's name-like (PROPN or part of a PERSON entity)
        """
        ...
        try:
            ...
            for tok in doc:
                t = tok.text
                if not t or t not in self.AMBIGUOUS_NAME_WORDS:
                    continue
                if not t[:1].isupper():
                    continue
                # Mark only if it's name-like in context
                if tok.pos_ == "PROPN" or tok.ent_type_ == "PERSON":
                    self.ambiguous_name_tokens.add(t)
        except Exception:
            return
        ...
        for chunk in chunks:
            ...
            for ent in doc.ents:
                ...
                if ent.label_ == "PERSON":
                    ...
                    # v1.18.0: Never treat tribe/ethnonym strings as PERSON (DB-driven).
                    if self._is_tribe_term(ent_text):
                        continue

                    # v1.18.1: Ambiguous "name-like" common words (Will/May/etc).
                    # If spaCy tags one of these as PERSON and it appears capitalized, mark for manual review
                    # rather than replacing it with a code.
                    words = ent_text.split()
                    if len(words) == 1 and ent_text[:1].isupper() and ent_text in self.AMBIGUOUS_NAME_WORDS:
                        self.ambiguous_name_tokens.add(ent_text)
                        continue
                    
                    # v1.17.8: Tighten PERSON acceptance to keep transcripts readable.
                    # We only accept:
                    # - multi-word names (2-4 tokens), or
                    # - single-token names that are validated AND appear in strong name-introducing context
                    # This prevents nouny false positives like "State", "Project", etc. becoming person codes.
                    # (words already computed above)
                    if len(words) == 1:
                        # Reject most single-token persons unless clearly introduced as a name in nearby context.
                        start = max(0, ent.start_char - 40)
                        end = min(len(chunk), ent.end_char + 40)
                        ctx = chunk[start:end].lower()
                        if not re.search(r"\b(my\s+name\s+is|i\s+am|i'm|this\s+is|sorry,|thank\s+you,|mr\.|ms\.|mrs\.|dr\.)\b", ctx):
                            continue
                        if not self.is_valid_name(ent_text):
                            continue
    ...
    def is_valid_name(self, name: str) -> bool:
        ...
        # v1.18.1: Single-token names are the biggest source of "One/Farmer/Doing" false positives.
        # Only accept a single-token PERSON if the DB says it's a real first-name entry.
        if len(words) == 1 and not db_hit_single:
            return False
    ...
    def extract_entities(self, text: str) -> Dict[str, List[str]]:
        ...
        for pattern in person_patterns:
            ...
            for match in matches:
                ...
                if name.lower().startswith("the "):
                    name = name[4:].strip()
                # v1.18.0: Never treat tribe/ethnonym terms as persons.
                if self._is_tribe_term(name):
                    continue
                # v1.18.1: Mark ambiguous common-word names for manual review instead of coding.
                if name in self.AMBIGUOUS_NAME_WORDS:
                    self.ambiguous_name_tokens.add(name)
                    continue
                # Never treat the token "Name" as a person (common spaCy/regex failure mode)
        ...
        for pattern in last_name_patterns:
            ...
            for match in matches:
                ...
                if self._is_tribe_term(name_normalized):
                    continue
                if name_normalized in self.AMBIGUOUS_NAME_WORDS:
                    self.ambiguous_name_tokens.add(name_normalized)
                    continue
                if name_normalized.strip().lower() == "name":
                    continue
    ...
    def create_codes(self, entities: Dict[str, List[str]]):
        ...
        # v1.17.7: People use compact, human-readable codes: #A, #B, ... #Z, #AA, #AB, ...
        # If a person is also a speaker, their speaker letter (A/B/AA/...) matches their # code.
        ...
        def _person_code_from_counter(counter_value: int) -> str:
            # counter_value is 1-based; labels are 0-based.
            return f"#{_label_from_index(counter_value - 1)}"
        
        # Process persons
        seen_canonicals = set()

        # Stable ordering: most frequent canonicals first, then alphabetically.
        canonical_counts = {}
        try:
            for variant, canonical in canonical_mapping.items():
                canonical_counts[canonical] = canonical_counts.get(canonical, 0) + int(self.name_detector.name_counter.get(variant, 0))
        except Exception:
            canonical_counts = {}

        canonicals = list({c for c in canonical_mapping.values() if c})
        canonicals.sort(key=lambda c: (-canonical_counts.get(c, 0), c.lower()))

        for canonical in canonicals:
            if canonical not in seen_canonicals:
                self.person_counter += 1
                code = _person_code_from_counter(self.person_counter)
        ...
        # v1.18.0: Collapse bare first-name variants onto a unique full-name code when possible.
        # This prevents unreadable splits like "#U #K" for what should be one person ("Jodi Burshia").
        full_first_to_codes = {}
        for name, code in list(self.mapping["persons"].items()):
            if not isinstance(name, str):
                continue
            if " " in name.strip():
                first = name.strip().split()[0].lower()
                full_first_to_codes.setdefault(first, set()).add(code)

        unique_first_code = {first: next(iter(codes)) for first, codes in full_first_to_codes.items() if len(codes) == 1}
        for name in list(self.mapping["persons"].keys()):
            if not isinstance(name, str):
                continue
            if " " in name.strip():
                continue
            first = name.strip().lower()
            if first in unique_first_code:
                self.mapping["persons"][name] = unique_first_code[first]
    ...
    def deidentify_text(self, text: str, remove_timestamps: bool = True, format_dialogue: bool = True, 
        ...
        def _dbg(msg: str) -> None:
            try:
                print(f"    [deidentify_text] {msg}", flush=True)
            except Exception:
                pass

        deidentified = text
        _dbg(f"start len={len(deidentified)}")
        
        # First, correct misspellings
        deidentified = correct_misspellings(deidentified)
        # v1.18.0: Fix a few common tribe self-ID typos (contextual, minimal).
        deidentified = self._fix_common_tribe_misspellings(deidentified)
        _dbg(f"after misspellings dt={time.time()-t0:.2f}s")

        # v1.18.2: Handle ambiguous tokens (Will/May/etc.) per policy.
        # We bracket for manual review to avoid destroying meaning when the token could be a normal word.
        if self.ambiguous_policy == "mark_all":
            # PERFORMANCE FIX v1.18.7:
            # mark_all used to run one full-text regex substitution per ambiguous token, which can
            # be extremely slow on large transcripts. Do a single-pass alternation replacement.
            try:
                toks = [t for t in sorted(self.AMBIGUOUS_NAME_WORDS) if t and t[:1].isupper()]
                if toks:
                    amb_pat = re.compile(
                        r"(?<!\\[)\\b(" + "|".join(re.escape(t) for t in toks) + r")\\b(?!\\])"
                    )
                    deidentified = amb_pat.sub(lambda m: f"[{m.group(1)}]", deidentified)
            except Exception:
                pass
        else:
            # pos_based: only bracket when spaCy/NER indicates name-like usage.
            self._collect_ambiguous_tokens_pos_based(deidentified)
            try:
                for tok in sorted(self.ambiguous_name_tokens):
                    if not tok or not tok[:1].isupper():
                        continue
                    pat = re.compile(rf"(?<!\\[)\\b{re.escape(tok)}\\b(?!\\])")
                    deidentified = pat.sub(f"[{tok}]", deidentified)
            except Exception:
                pass
        _dbg(f"after ambiguous_policy={self.ambiguous_policy} dt={time.time()-t0:.2f}s")
        ...
        if remove_timestamps and not use_citation_system:
            ...
            deidentified = remove_webvtt_timestamps(deidentified)
        _dbg(f"after remove_timestamps dt={time.time()-t0:.2f}s")
        ...
        _dbg(f"person_items={len(person_items)} dt={time.time()-t0:.2f}s")
        ...
        # Replace full names — HARD PERFORMANCE FIX v1.18.5
        #
        # We observed a real-world stall at "De-identifying and formatting..." for large DOCX inputs.
        # Root cause: exploding name-variant mappings + many repeated full-text regex passes.
        #
        # v1.18.5 strategy: ONLY replace the extracted entity strings we decided were people.
        # - Misspellings are already normalized by correct_misspellings() earlier.
        # - Ambiguous tokens are bracketed earlier ("[Will]") and must not be replaced.
        #
        # This makes runtime effectively proportional to transcript size + #people (small), and avoids
        # accidental replacement of non-people tokens that slipped into the canonical mapping.
        if person_items:
            names_sorted = sorted({n.strip().lower() for (n, _c) in person_items if n and n.strip()}, key=len, reverse=True)
            name_to_code_lc = {n.strip().lower(): c for (n, c) in person_items if n and n.strip() and c}

            if names_sorted:
                name_pat = re.compile(
                    r"(?<!\[)(?<![A-Za-z])(" + "|".join(re.escape(v) for v in names_sorted) + r")(?![A-Za-z])(?!\])",
                    flags=re.IGNORECASE,
                )

                def _repl_name(m):
                    return name_to_code_lc.get(m.group(1).lower(), m.group(0))

                deidentified = name_pat.sub(_repl_name, deidentified)
        _dbg(f"after person full-name replace dt={time.time()-t0:.2f}s")
        ...
        _dbg(f"after first-name replace dt={time.time()-t0:.2f}s")
        ...
        _dbg(f"after org replace ({len(org_items)}) dt={time.time()-t0:.2f}s")
        ...
        for original, code in loc_items:
            pattern = r'\b' + re.escape(original) + r'\b'
            deidentified = re.sub(pattern, code, deidentified, flags=re.IGNORECASE)
        _dbg(f"after loc replace ({len(loc_items)}) dt={time.time()-t0:.2f}s")
        ...
        for original, code in tribe_items:
            pattern = r'\b' + re.escape(original) + r'\b'
            deidentified = re.sub(pattern, code, deidentified, flags=re.IGNORECASE)
        _dbg(f"after tribe replace ({len(tribe_items)}) dt={time.time()-t0:.2f}s")
                ...
                if any(name.lower() == e.lower() for e in self.mapping["persons"].keys()):
                    ...
                    def _label_from_index(i: int) -> str:
                        ...
                        while True:
                            ...
                            i -= 1
                        return ''.join(reversed(out))
                    found_code = f"#{_label_from_index(self.person_counter - 1)}"
        ...
        _dbg(f"after known_names_to_replace dt={time.time()-t0:.2f}s")
        ...
        if format_dialogue:
            ...
            speaker_mapping = {}
            # v1.17.7: speaker roles are no longer printed; keep minimal metadata.
            for code in set(self.mapping["persons"].values()):
                speaker_mapping[code] = "Speaker"
            
            if use_citation_system:
                _dbg("before format_with_citation_system()")
                ...
                _dbg("after format_with_citation_system()")
            else:
                deidentified = format_as_dialogue(deidentified, speaker_mapping)
        _dbg(f"after formatting dt={time.time()-t0:.2f}s")
        ...
        for name in known_names_to_replace:
            ...
            if found_code:
                ...
                for pattern in patterns:
                    deidentified = re.sub(pattern, found_code, deidentified, flags=re.IGNORECASE)
        _dbg(f"after post-format known_names dt={time.time()-t0:.2f}s")
        
        # REMOVED v1.16.0: Hardcoded location replacement - location replacement above should handle all locations generically
        
        # v1.18.0: Final cleanup for readability after ALL replacement passes.
        # - Collapse repeated person codes: "#D #D" -> "#D"
        deidentified = re.sub(r"(#[A-Z]{1,3})(?:\s+\1)+", r"\1", deidentified)
        # - Drop stray leading person code before common greetings (usually duplication from name-intro extraction).
        try:
            cleaned_lines = []
            greet_re = re.compile(r"^(\[[A-Z]{1,3}\.\d+\]\s+)#([A-Z]{1,3})\s+(?=(Good\s+morning|Good\s+afternoon|Good\s+evening|Hello|Hi|Hey)\b)", re.IGNORECASE)
            for ln in deidentified.splitlines():
                cleaned_lines.append(greet_re.sub(r"\1", ln))
            deidentified = "\n".join(cleaned_lines)
        except Exception:
            pass
        _dbg(f"done dt={time.time()-t0:.2f}s len={len(deidentified)}")
...
class KeywordTagger:
    ...
    def tag_text(self, text: str) -> Dict[str, List[Tuple[int, str, str]]]:
        ...
        # First pass: Tag obvious lines (person codes, speaker labels, verses)
        for line_num, line in enumerate(lines, 1):
            ...
            if line_num not in tagged_lines:
                # Tag lines with person codes as dialogue (#A/#AA etc)
                if re.search(r'#[A-Z]{1,3}\b', line):
...
def process_transcript(input_path: Path, output_dir: Path, use_spacy: bool = True,
                      use_citation_system: bool = True, lines_per_page: int = DEFAULT_LINES_PER_PAGE,
                      ambiguous_policy: str = "mark_all") -> Dict:
    ...
    deidentifier.ambiguous_policy = (ambiguous_policy or "mark_all")
...
Examples:
  ...
  python deidentify_and_tag_transcripts_v1.7.0.py --lines-per-page 40
    ...
    parser.add_argument(
        '--ambiguous-policy',
        choices=['mark_all', 'pos_based'],
        default='mark_all',
        help='How to handle ambiguous common-word tokens like Will/May. '
             'mark_all brackets any capitalized occurrence; pos_based uses spaCy context.'
    )
    
    args = parser.parse_args()
    
    print("=" * 80)
    print("DE-IDENTIFY AND TAG TRANSCRIPTS v1.18.7")
    print("=" * 80)
