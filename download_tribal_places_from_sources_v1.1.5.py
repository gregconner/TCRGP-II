#!/usr/bin/env python3
"""
Download Comprehensive Tribal Place Names from Public Sources v1.1.5
...
def _download_bytes_cached(url: str, label: str, timeout_s: float, attempts: int, refresh: bool = False) -> bytes:
    ...
    if cache_bin.exists() and not refresh:
        try:
            ...
            if PROGRESS_SINK is not None and hasattr(PROGRESS_SINK, "update_subtask"):
    ...
    # If a parent is managing a 3-line no-scroll UI, don't run our own ticker.
    prev = None if (PROGRESS_SINK is not None) else _start_per_second_progress_timer(_tick)
    try:
